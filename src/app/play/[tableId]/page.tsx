...
export default function PlayPage() {
  ...
  const { game, isConnected, sendAction } = useSocketGame(tableId);

  const heroPlayer = game?.players.find(p => p.id === user?.id);

  if (isUserLoading || !game || !user) {
    ...
    <div className="flex flex-col lg:flex-row h-[calc(100vh-4rem)] bg-background overflow-hidden">
      ...
      <main className="flex-grow flex flex-col">
        ...
        <div className="p-4 flex flex-col items-center gap-4">
          ...
          <ActionButtons playerStack={heroPlayer.stack} currentBet={0} minRaise={2000} onAction={sendAction} />
        </div>
      </main>
