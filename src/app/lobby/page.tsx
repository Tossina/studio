        ...
        <div className="space-y-1">
          {gameTypes.map(({ id, name, icon: Icon }) => (
            <button
              ...
              className={`w-full flex items-center gap-3 p-2 rounded-md text-sm font-medium transition-colors ${activeType === id ? 'bg-accent text-foreground' : 'hover:bg-accent/50'
                }`}
            >
...
export default function LobbyPage() {
  ...
  const fetchGames = () => {
    ...
    fetch('/api/games')
      ...
      .catch(err => setGamesLoading(false));
  };
    ...
    <div className="container mx-auto py-8">
      <Dialog open={openCreateGame} onOpenChange={setOpenCreateGame}>
        <DialogContent>
          ...
          <CreateGameForm setOpen={setOpenCreateGame} onSuccess={fetchGames} />
        </DialogContent>
      </Dialog>
